...
/// The main interface for parking and retrieving objects. Anything stored at the top level directly must conform to `Codable` and `Identifiable`, or GarageStorage's ``Mappable`` protocol.
///
...
///
/// Any object going into or coming out of the `Garage` must conform to the `Codable` protocol. Top-level objects must also conform to the `Identifiable` protocol, or the ``Mappable`` convenience protocol (which is `Codable & Identifiable`) for uniquely identified top-level objects.
///
...
public class Garage: NSObject {
    ...
    public static func makePersistentStoreDescription(_ storeName: String) -> NSPersistentStoreDescription {
        ...
        let storeURL = applicationDocumentsDirectory.appending(path: storeName)
    ...
    internal func fetchCoreDataObject(for type: String, identifier: String) throws -> CoreDataObject {
        guard let coreDataObject = fetchObject(for: type, identifier: identifier) else {
            throw GarageError.failedToRetrieveObject(type, identifier)
        }
        return coreDataObject
    }

	/// Fetches objects matching the specified syncStatus, optionally with the specified type string.
	/// Must be called from within context.performAndWait.
